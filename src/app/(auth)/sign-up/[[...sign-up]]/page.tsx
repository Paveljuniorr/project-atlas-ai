...
export default function SignUpPage() {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      setLoading(false);
      setError(err?.errors?.[0]?.message || err.message || "Failed to initiate Google sign-up.");
    }
  };
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-slate-50 text-slate-900 p-4">
      ...
      <div className="mb-6 text-center max-w-md">
        ...
        <p className="text-slate-500 text-sm">
          Initialize your organization and AI pipeline with Google
        </p>
      </div>
      ...
      <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-7 shadow-xl">
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <span>{loading ? "Initializing Workspace..." : "Continue with Google"}</span>
          </button>
        </div>
      ...
      <p className="mt-8 text-center text-xs text-slate-400 max-w-xs">
        Automatic workspace provisioning enabled. No password required.
      </p>
    </div>
  );
}
